function searchModel() {
    ...
    if (keyword) {
        ...
        // 直连用户搜索直达原站列表页，搜索流量也绕过镜像；
        // 探测未完成时回退镜像列表页（卡片仍会被 direct-links.js 改写）
        const direct = directHF || sessionStorage.getItem('hfDirect') === '1';
        const base = direct ? 'https://huggingface.co/models'
                            : `https://${window.location.hostname}/models`;
        window.location.href = `${base}?search=${q}`;
    }
}
...
// 文本节点与链接 href 一并改写（含指向本站的链接与经代理的文档路径）。
document.addEventListener("DOMContentLoaded", function () {
    const host = window.location.hostname;
    const instructions = document.querySelector('.instructions');
    if (!instructions) return;

    const walker = document.createTreeWalker(instructions, NodeFilter.SHOW_TEXT);
    let node;
    while ((node = walker.nextNode())) {
        if (node.nodeValue.includes('hf-mirror.com')) {
            node.nodeValue = node.nodeValue
                .replaceAll('https://hf-mirror.com', `https://${host}`)
                .replaceAll('hf-mirror.com', host);
        }
    }
    // 链接 href 同步本地化（本站链接、/docs/ 等经代理路径）
    instructions.querySelectorAll('a[href*="hf-mirror.com"]').forEach(a => {
        a.href = a.href
            .replaceAll('https://hf-mirror.com', `https://${host}`)
            .replaceAll('hf-mirror.com', host);
    });
...
// ---------- 直连检测 ----------
// 能直接访问 huggingface.co 的用户（含走系统代理者），热门卡片直达原站详情页，
// 详情页流量绕过镜像，避免镜像出口 IP 被 HF 限流（429）。
// 探测用 no-cors 请求：被墙时连接被重置/超时 -> fetch reject；可达 -> resolve。
let directHF = false;

function rewriteCardLinksToOrigin() {
    // 热门卡片：详情页直达原站
    document.querySelectorAll('.repo-link').forEach(a => {
        const path = a.getAttribute('href');
        if (path && !path.startsWith('http')) {
            a.href = 'https://huggingface.co/' + path.replace(/^\//, '');
        }
    });
    // 教程区文档链接：文档页直达原站（仅改写指向本站的 /docs/ 链接；
    // 用 hostname 比较而非 origin，避免本地化器生成无端口 URL 时误判）
    document.querySelectorAll('.instructions a[href*="/docs/"]').forEach(a => {
        if (a.hostname === location.hostname) {
            a.href = 'https://huggingface.co' + a.pathname + a.search;
        }
    });
}

async function detectDirectHF() {
    // 30 分钟内使用缓存结果，不重复探测
    const expire = sessionStorage.getItem('hfDirectExpire');
    if (expire && Date.now() < Number(expire)) {
        directHF = sessionStorage.getItem('hfDirect') === '1';
        if (directHF) rewriteCardLinksToOrigin();
        return;
    }
    ...
    try {
        ...
        clearTimeout(timer);
        sessionStorage.setItem('hfDirectExpire', String(Date.now() + 30 * 60 * 1000));
    }
    if (directHF) rewriteCardLinksToOrigin();
}

document.addEventListener('DOMContentLoaded', detectDirectHF);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getTrendingItemHTML(item, index) {
        ...
        const repoPath = item.repoType === 'model' ? repoId : `/${item.repoType}s/${repoId}`;
        const repoLink = directHF ? `https://huggingface.co/${repoPath.replace(/^\//, '')}` : repoPath;
                        ...
                        <div class="other-stats">
                            ...
                            <div class="stats-icons">
                                ...
                                <span class="likes">
                                    ...
                                    <span class="count">${abbreviateNumber(item.repoData.likes)}</span>
                                </span>
                            </div>
                        </div>
                    </div>
